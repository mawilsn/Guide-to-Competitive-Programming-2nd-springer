# g++ -std=c++11 -O2 -Wall test.cpp -o test
# ./test


# g++ -std=c++11 -O2 -Wall input.cpp -o input
# ./input 


# g++ -std=c++11 -O2 -Wall read_input.cpp -o read_input
# ./read_input 

# g++ -std=c++11 -O2 -Wall read_line.cpp -o read_line
# ./read_line

# g++ -std=c++11 -O2 -Wall freopen_example.cpp -o freopen_example
# ./freopen_example

# g++ -std=c++11 -O2 -Wall int_long_bug.cpp -o int_long_bug && ./int_long_bug

# g++ -std=c++11 -O2 -Wall generate_subset.cpp -o generate_subset && ./generate_subset

# g++ -std=c++11 -O2 -Wall permutations.cpp -o permutations && ./permutations

g++ -std=c++11 -O2 -Wall bit_represnting.cpp -o bit_represnting && ./bit_represnting